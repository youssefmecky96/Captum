...
def apply_gradient_requirements(
    ...
    for index, input in enumerate(inputs):
        assert isinstance(
            input, torch.Tensor), "Given input is not a torch.Tensor"
...
def compute_gradients(
    ...
    with torch.autograd.set_grad_enabled(True):
        ...
        grads = torch.autograd.grad(torch.unbind(
            outputs), inputs, allow_unused=True)
    return grads
...
def _forward_layer_distributed_eval(
    ...
    """
    saved_layer: Dict[Module, Dict[device,
                                   Tuple[Tensor, ...]]] = defaultdict(dict)
    lock = threading.Lock()
    ...
    try:
        for single_layer in all_layers:
            if attribute_to_layer_input:
                all_hooks.append(
                    single_layer.register_forward_pre_hook(
                        hook_wrapper(single_layer))
                )
            else:
                all_hooks.append(
                    single_layer.register_forward_hook(
                        hook_wrapper(single_layer))
                )
        output = _run_forward(
            forward_fn,
        ...
        if isinstance(layer, Module):
            ...
            return [
                _gather_distributed_tensors(
                    saved_layer[curr_layer], key_list=key_list)
...
def compute_layer_gradients_and_eval(
    ...
    with torch.autograd.set_grad_enabled(True):
        ...
        all_layers: List[Module] = [layer] if isinstance(
            layer, Module) else layer
        grad_inputs = tuple(
            layer_tensor
            for single_layer in all_layers
            for device_id in key_list
